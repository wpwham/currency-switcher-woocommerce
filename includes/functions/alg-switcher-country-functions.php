...
if ( ! function_exists( 'alg_get_country_flag_code' ) ) {
	 ...
	 * @since   2.8.3
	 */
	function alg_get_country_flag_code( $currency_code ) {
		if ( '' === ( $flag_code = get_option( 'alg_wc_currency_switcher_flags_' . $currency_code, '' ) ) ) {
			$currency_countries = alg_get_currency_countries();
			if ( false === ( $flag_code = ( isset( $currency_countries[ $currency_code ][0] ) ? $currency_countries[ $currency_code ][0] : false ) ) ) {
				$crypto_icons = alg_get_crypto_icons();
				$flag_code = ( isset( $crypto_icons[ $currency_code ] ) ? $crypto_icons[ $currency_code ] : 'no-flag' );
			}
		}
		return $flag_code;
	}
...
if ( ! function_exists( 'alg_get_country_flag_image_url' ) ) {
	 ...
	 * @version 2.8.3
	 * @since   2.4.4
	 * @todo    ensure all flag images exist
	 * @todo    removed: `return ( @getimagesize( $file_url ) ? $file_url : alg_wc_currency_switcher_plugin()->plugin_url() . '/assets/images/flag-icons/' . 'no-flag' . '.png' );`
	 */
	function alg_get_country_flag_image_url( $country_code ) {
		return apply_filters( 'alg_wc_currency_switcher_country_flag_image_url',
			alg_wc_currency_switcher_plugin()->plugin_url() . '/assets/images/flag-icons/' . strtolower( $country_code ) . '.png' );
	}
}

if ( ! function_exists( 'alg_get_crypto' ) ) {
	/**
	 * alg_get_crypto.
	 *
	 * @version 2.8.3
	 ...
	 * @return  array
	 */
	function alg_get_crypto() {
		return array(
			'bitcoin'      => __( 'Bitcoin', 'currency-switcher-woocommerce' ),
			'bitcoin-cash' => __( 'Bitcoin Cash', 'currency-switcher-woocommerce' ),
			'ethereum'     => __( 'Ethereum', 'currency-switcher-woocommerce' ),
			'litecoin'     => __( 'Litecoin', 'currency-switcher-woocommerce' ),
		);
	}
}

if ( ! function_exists( 'alg_get_crypto_icons' ) ) {
	/**
	 * alg_get_crypto_icons.
	 *
	 * @version 2.8.3
	 * @since   2.8.3
	 * @return  array
	 */
	function alg_get_crypto_icons() {
		return array(
			'BTC' => 'bitcoin',
			'BCH' => 'bitcoin-cash',
			'ETH' => 'ethereum',
